...
| `src/rqdata_tick_data/cli.py` | handler 已拆出，`build_parser` 仍集中注册全部命令。 | 按命令域拆 parser 注册函数，保持 CLI 参数兼容。 | CLI help 和离线 smoke 测试。 |
| `src/rqdata_tick_data/health.py` | 全量扫描仍在内存保留完整 symbol-date diagnostics。 | 默认报告保留汇总和异常样例；完整诊断流式写 CSV/Parquet。 | health 诊断测试；大池峰值内存检查。 |
