...
| 用途 | RQData 能力 | 项目约定 |
| --- | --- | --- |
| Tick-depth 下载 | `rqdatac.get_price(..., frequency="tick", market="hk", expect_df=True)` | 用 `symbol-date` raw layout 保存，不重建订单簿 |
...
| 日频 reference | `get_price(..., frequency="1d", market="hk")` | 只用于 raw-daily gate 或 cross-clean 对照 |
...
`volume` 和 `total_turnover` 按累计字段处理；日频特征由 `aggregate-daily` 从 raw tick
